...
def farm_1(this_map=maps.farm_1.copy()):
    ...
    for i in range(len(towers_to_build)):
        ...
        towers_info[tower] = {}
        towers_info[tower]['cells'] = []
        for j in range(len(this_map['towers'][tower]['cells'])):
            x, y = this_map['towers'][tower]['cells'][j]
            towers_info[tower]['cells'].append((x, y))

    towers_order = []
    for i in range(max([len(towers_info[tower]['cells']) for tower in towers_info])):
        for tower in towers_info:
            if len(towers_info[tower]['cells']) % (i+1) == 0:
                towers_order.append(tower)
    towers = {}
    mods = {}
    ...
    while True:
        ...
        while len(tasks_keys) > 0:
            ...
            offset = update_offset(rewards_to_build, rewards, offset, task['cost'])
            click(grid[cell])
        ...
        for tower in first_towers:
            ...
            for upgrade in info.towers[tower]['num_upgrades']:
                press_key(['ctrl', upgrade], 1, 0, 0.1)
